...
namespace RopeMinikit
{
    public class Rope : MonoBehaviour
    {
        ...
        [HideInInspector]
        public bool once,control;
        [HideInInspector]
        public int choosenParticle;
        [HideInInspector]
        ...
        public void UpdateCollisionPlanes()
        {
            ...
            for (int i = 0; i < activeCollisionPlanes.Length; i++)
            {
                ...
                for (int j = 0; j < hitCount && planeCount < MaxCollisionPlanesPerParticle; j++)
                {
                    ...
                    //{
                    //    GetClosestParticle(collider.transform.position, out int particleIndex, out float distance);
                    //    collider.transform.position = GetPositionAt(particleIndex);
                    ...
                    if (collider.tag=="Player")
                    {
                        ...
                        if (particleIndex>30)
                        {
                            ...
                            SetVelocityAt(particleIndex, 0);
                            SetVelocityAt(0, 0);
                            choosenParticle = particleIndex;
                            choosenObject = collider.gameObject;
                            control = true;
                            for (int t = particleIndex; t <0 ; t--)
                            {
                                Debug.Log(t);
                                //SetMassMultiplierAt(t, 0);
                                SetVelocityAt(t, 0);
                            }
        ...
        public void FixedUpdate()
        {
            ...
            if (control)
            {
                SetPositionAt(choosenParticle, choosenObject.transform.position);
            }
